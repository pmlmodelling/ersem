...
contains
   ...
   subroutine initialize(self,configunit)
      ...
      call self%get_parameter(rRPmX,'rm',default=0.0_rk)
      call self%get_parameter(EPS,'EPS',default=0.0_rk)
      call self%get_parameter(c0,'c0',default=0.0_rk)

      call self%initialize_ersem_base(rm=rRPmX)
      
      call self%get_parameter(composition,'composition',default='')
      ...
      if (index(composition,'s')/=0) then
         call self%get_parameter(s0,'s0',default=qsRPIcX*c0)
   ...
   subroutine initialize_ersem_base(self,rm,sedimentation)
      ...
      if (present(sedimentation)) then
         ...
         if (sedimentation) call self%get_parameter(self%sedimentation,'sedimentation',default=.true.)
      else
         ! No sedimentation flag provided; get from run-time configuration.
         call self%get_parameter(self%sedimentation,'sedimentation',default=.false.)
      end if
